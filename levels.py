...
class Levels( object ) :
    ...
    dummy_number: a number of dummy observations equal to the average added to each level group in order to mitigate the effect of rare levels (levels with a few
    ...
    levels = Levels( features=[ 'a' ], label='b', sample_frac=0.7, dummy_number=1 )
    levels.fit( d )    
    print( levels.vars_[ 'a' ] )
    transformed = levels.transform( d ) """
